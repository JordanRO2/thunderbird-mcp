      ...
      mcpServer: {
        start: async function() {
          ...
          try {
            ...
            //
            // The contacts, calendar, and filters tool implementations live in
            // mcp_server/domain/*.js. They are loaded here with the same
            // loadSubScript + module.exports shim used for security_helpers.js,
            // because a WebExtension Experiment API loads a single parent script
            // (this file) and supports neither a multi-script list nor ES module
            // import. Each module exports register(ctx): it reads the shared
            // bindings it needs from ctx and assigns its tool functions back onto
            // ctx. We load them at this point -- after every shared constant,
            // XPCOM binding, and infra helper is in scope -- then pull the tool
            // functions into local scope so the dispatch switch below can call
            // them by their original bare names.
            const __domainCtx = {
              Cc, Ci, Services, MailServices,
              cal, CalEvent, CalTodo, GlodaMsgSearcher,
              MAX_SEARCH_RESULTS_CAP, DEFAULT_MAX_RESULTS,
              appendComposeAudit, isContactWritesBlocked, isSkipReviewBlocked,
              isFilterForwardReplyBlocked, isAccountAllowed,
              getAccessibleFolder, getAccessibleAccounts,
            };
            for (const __mod of ["domain/contacts.js", "domain/calendar.js", "domain/filters.js"]) {
